      ...
      <b-row>
        ...
        >
          <h1 class="font-weight-bold text-info">
            Pagos Pendientes de los Ruteros
            <feather-icon
              icon="DollarSignIcon"
              size="1.1x"
            />
          ...
          >
            <template #cell(id)="data">
              <b-link
                ...
                :to="{ name: 'invoice-carrier', params: { id: data.item.id }}"
...
export default {
  ...
  methods: {
    /// ////////////  Funcion Mostrar //////////////////
    getItems() {
      this.$store.dispatch('payment/getCarriers')
    },
    ...
    async paidProducer(item) {
      const res = await confirmAlert('¿Está seguro de Pagar a este Rutero?')
      console.log(res)
      if (res.value) {
    ...
    async allPaid() {
      ...
      console.log(res)
      if (res.value) {
        const r = await this.$store.dispatch('payment/generatePaymentCarrier')
        console.log(r)
