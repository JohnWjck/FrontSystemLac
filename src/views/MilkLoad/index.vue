...
import MilkLoadForm from './components/MilkLoadForm.vue'
...
export default {
  ...
  computed: {
    ...mapState({
      items: state => state.milkLoad.items,
    }),
    rows() {
  ...
  methods: {
    /// ////////////  Funcion Mostrar //////////////////
    getItems() {
      this.$store.dispatch('milkLoad/getToday')
    },
    filterTable(item, filter) {
      const searchTerm = filter.toLowerCase()
      return (
        item.id.toString().includes(searchTerm)
        || item.producer.name.toLowerCase().includes(searchTerm)
        || item.carrier.name.toLowerCase().includes(searchTerm)
      )
    },
    edit(item) {
      this.item = item
