...
export default {
  ...
  actions: {
    ...
    async get({ commit }) {
      try {
        await API.get('milk-loads').then(response => {
    ...
    async getToday({ commit }) {
      try {
        await API.get('milk-loads/today').then(response => {
          commit('SET_ITEMS', response.data)
        })
      } catch (error) {
        console.log(error)
    ...
    async save({ commit, state }, payload) {
      try {
        const response = await API.post('milk-loads/create', payload)
    ...
    async delete({ commit, state }, payload) {
      try {
        console.log(payload)
        const { data: response } = await API.delete('milk-loads/delete', {
          params: payload,
        })
        if (response.success) {
